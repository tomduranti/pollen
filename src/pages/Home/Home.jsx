...
export default function Home({ defaultOrUserLocale, userId }) {
    ...
    let filteredPollenList = [];
    let activePollen = [];

    usePollenSync(userId, defaultOrUserLocale, pollenData, setPollenData, setUserLocation, setError);
    
    if (pollenData?.length > 0) {

        filteredPollenList = pollenData?.map(item => {
        ...
        activePollen = filteredPollenList[0]?.filter(item => item !== null).length;
    }
    ...
    return (
        <>
            {error
                ...
                :  pollenData?.length > 0
                        ? (
                            <section className='flex flex-col gap-5 md:mx-auto md:max-w-lg md:w-full'>
                                <Link to='/location' tabIndex={0} className='safari flex gap-1.5'>
                        ...
                        : <LoaderCircle className='animate-spin m-auto' color='#2E7D57E6' size={42} />
            }
        </>
    )
}